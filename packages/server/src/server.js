...
/**
 * @typedef {function(React.ReactElement): Array<{key: string, ids: Array<string>, css: string}>} EmotionCriticalToChunks
 */

/**
 * @typedef {function(Array<{key: string, ids: Array<string>, css: string}>): string} EmotionConstructStyleTags
 */

/**
 * @typedef {Object} EntryServer
 * @property {function(string): React.ReactElement} renderPage
 * @property {function(string): React.ReactElement} [renderToStream]
 * @property {EmotionCriticalToChunks} extractCriticalToChunks
 * @property {EmotionConstructStyleTags} constructStyleTagsFromChunks
 */
...
let renderPage = () => null;
let renderToStream = () => null;
let extractCriticalToChunks = null;
let constructStyleTagsFromChunks = null;
...
if (fs.existsSync(clientDistPath)) {
  ...
  app.use(sirv(clientDistPath, {
    ...
    etag: true,
    maxAge: process.env.NODE_ENV === 'development' ? 0 : 31536000, // 1 year for production
  ...
  try {
    ...
    try {
      if (fs.existsSync(ssrEntryPath)) {
        ...
        renderPage = entryServer.renderPage || (entryServer.default && entryServer.default.renderPage) || (() => null);
        renderToStream = entryServer.renderToStream || (entryServer.default && entryServer.default.renderToStream) || renderPage;
        extractCriticalToChunks = entryServer.extractCriticalToChunks || (entryServer.default && entryServer.default.extractCriticalToChunks) || null;
        constructStyleTagsFromChunks = entryServer.constructStyleTagsFromChunks || (entryServer.default && entryServer.default.constructStyleTagsFromChunks) || null;
      } else {
        ...
        if (fs.existsSync(customEntryPath)) {
          ...
          renderPage = customEntry.renderPage || (customEntry.default && customEntry.default.renderPage) || (() => null);
          renderToStream = customEntry.renderToStream || (customEntry.default && customEntry.default.renderToStream) || renderPage;
          extractCriticalToChunks = customEntry.extractCriticalToChunks || (customEntry.default && customEntry.default.extractCriticalToChunks) || null;
          constructStyleTagsFromChunks = customEntry.constructStyleTagsFromChunks || (customEntry.default && customEntry.default.constructStyleTagsFromChunks) || null;
        }
      }
    } catch (importError) {
