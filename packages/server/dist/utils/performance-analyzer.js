...
const arrayManipulationTest = async () => {
    ...
    return compareImplementations('Array Manipulation', standardArrayManipulation, optimizedArrayManipulation, 50);
};
...
const parallelProcessingTest = async () => {
    ...
    const optimizedParallelProcessing = async () => {
        const results = await async.mapLimit(items, 10, // Concurrency limit
        async (item) => simulateAsyncOperation(item));
        return results.length;
    };
    return compareImplementations('Parallel Processing', standardParallelProcessing, optimizedParallelProcessing, 5);
};
...
const objectManipulationTest = async () => {
    ...
    return compareImplementations('Object Manipulation', standardObjectManipulation, optimizedObjectManipulation, 100);
};
...
const runAllTests = async () => {
    ...
    try {
        ...
        fs.writeFileSync(path.join(reportsDir, 'performance-report.json'), JSON.stringify(report, null, 2));
...
class PerformanceAnalyzer {
    ...
    getMetrics(operation) {
        ...
        return {
            count: stats.count,
            totalTime: Number(stats.totalTime) / 1000000,
            avgTime: Number(stats.totalTime) / (stats.count * 1000000),
            min: Math.min(...durations) / 1000000,
            max: Math.max(...durations) / 1000000,
            p95: durations[p95Index] / 1000000,
            p99: durations[p99Index] / 1000000,
        };
    }
    getStats() {
        const stats = {};
        this.measurements.forEach((value, key) => {
            stats[key] = {
                count: value.count,
                totalTime: Number(value.totalTime) / 1000000,
                avgTime: Number(value.totalTime) / (value.count * 1000000),
            };
        });
        return stats;
    }
