...
const AIAnalyzer: React.FC<AIAnalyzerProps> = ({
  ...
  const handleAnalyze = async (): Promise<void> => {
    ...
    if (fromBranch === undefined || fromBranch === '' || toBranch === undefined || toBranch === '') {
  ...
  const handleFromBranchChange = (event: SelectChangeEvent<string>): void => {
    const selectedBranch = event.target.value;
    onFromBranchChange(selectedBranch);
  };

  const handleToBranchChange = (event: SelectChangeEvent<string>): void => {
    const selectedBranch = event.target.value;
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "flex-start" }}>
          <Button
            ...
            disabled={!repoPath || fromBranch === '' || toBranch === '' || isAnalyzing || isLoadingBranches}
            startIcon={<AIIcon />}
          >
