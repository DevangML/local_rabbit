 ...
 * Handles communication with the backend API for code review features
 */

/**
 * Service for code review API operations
 * @class
 */
class CodeReviewApiService {
  ...
  async selectRepository(path) {
    try {
      console.warn(`[CLIENT] Selecting repository at path: ${path}`);
      ...
      try {
        ...
        console.warn("[CLIENT] Repository selection successful");
        return response;
      } catch (error) {
        ...
        console.warn("[CLIENT] Repository selection successful (fallback)");
        return response;
      }
    } catch (error) {
  ...
  async getBranches(path) {
    try {
      ...
      console.warn(`[CLIENT] Fetching branches using URL: ${url}`);
