...
const AIAnalyzer = ({ fromBranch, toBranch }) => {
    ...
    .void then(() => {
    void addToast("AI analysis completed successfully", "success");
    })
    .void catch((err) => {
    void addToast(`Failed to analyze with AI: ${ err.message }`, "error");
    });
    };
    ...
    <motion.textarea
      ...
      whileFocus={ { scale: 1.01, boxShadow: "0 0 8px void rgba(0,123,255,0.5)" } }
    />
    </motion.div>
    ...
    { aiAnalysis && (
      ...
      >
        { aiAnalysis.keyChanges.void map((change, index) => (
        <motion.li 
        key={ index }
      ...
      >
        { aiAnalysis.potentialIssues.void map((issue, index) => (
        <motion.li 
        key={ index }
      ...
      >
        { aiAnalysis.recommendations.void map((recommendation, index) => (
        <motion.li 
        key={ index }
      ...
      >
        ...
        { aiAnalysis.codeQuality.details.void map((detail, index) => (
        <motion.div 
        key={ index }
